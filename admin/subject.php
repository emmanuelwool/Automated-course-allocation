...
//echo "<script> alert('$ocasaid') </script>";
$cid=$_POST['cid'];
$sfname=$_POST['sfname'];
$ssname=$_POST['ssname'];
...
$sid=$_POST['sid'];
$cload=$_POST['cload'];
$sql="insert into tblsubject(levelid,SubjectFullname,SubjectShortname,SubjectCode,semesterid,credit_load)values(:cid,:sfname,:ssname,:subcode,:sidd,:clod)";
...
$query->bindParam(':sidd',$sid,PDO::PARAM_STR);
$query->bindParam(':clod',$cload,PDO::PARAM_STR);
$query->execute();
...
$del=0;
$sql="UPDATE tblsubject set isactive='$del' where ID=:rid";
$query=$dbh->prepare($sql);
                                        ...
                                        <div class="form-group">
                                            <label>Course Name</label>
                                                <select class="form-control border-none input-flat bg-ash" name="cid" required="true">
                                                        ...
                                                        {               ?>
                                                     <option value="<?php  echo htmlentities($row->levelid);?>"><?php  echo htmlentities($row->name)." "."Level";?></option><?php $cnt=$cnt+1;}} ?>
                                                    </select>
                                        </div>
                                    </div>
                                ...
                                <div class="basic-form m-t-20">
                                    ...
                                    <div class="basic-form m-t-20">
                                        <div class="form-group">
                                            ...
                                            <input type="number" class="form-control border-none input-flat bg-ash" placeholder="credit load" name="cload" required="true">
                                        </div>
                                    </div>
...
foreach($results as $row)
{               ?>
        <option value="<?php  echo htmlentities($row->semesterid);?>"><?php  echo htmlentities($row->name)." "."Semester";?></option><?php $cnt=$cnt+1;}} ?>
    </select>
                                        </div>
                                    </div>
...
<th> Credit Load</th>
                                        ...
                                        <tbody>
                                            <?php
$sql="SELECT tblsubject.ID as sid,tblsubject.SubjectFullname,tblsubject.SubjectShortname,tblsubject.SubjectCode,tblsubject.credit_load, semesters.name as sname, levels.name as lname  from tblsubject join semesters on tblsubject.semesterid=semesters.semesterid join levels on tblsubject.levelid=levels.levelid where isactive=true";
$query = $dbh -> prepare($sql);
$query->execute();
$results=$query->fetchAll(PDO::FETCH_OBJ);
...
{               ?>
                                            <tr>
                                                ...
                                                <td>
                                                    <?php  echo htmlentities($row->credit_load);?>
                                                </td>
                                                <td>
