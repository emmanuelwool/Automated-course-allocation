...
//SELECT THE SUBJECTS
 $sql1="select ID from tblsubject where semesterid='$cid' AND isactive=True";
 $query1 = $dbh -> prepare($sql1);
 $query1->execute();
 $courses=$query1->fetchAll(PDO::FETCH_ASSOC);
  $count1=$query1->rowCount();

//SELECT THE LECTURERS 
$sql="select ID from tblteacher";
$query = $dbh -> prepare($sql);
$query->execute();
$lecturers=$query->fetchAll(PDO::FETCH_ASSOC);
 $count=$query->rowCount();

  // Iterate over lecturers and assign random courses
 $i=1;
 //while($i<=$count){
    foreach ($lecturers as $lecturer) {
        // Get a random number of courses to assign (e.g., between 1 and 3)
        $numCoursesToAssign = rand(2, 3);

        echo "<script> alert ('$numCoursesToAssign') </script>";

        // Shuffle the array of courses to randomize the selection
        //shuffle($courses);

        

        // Take the first $numCoursesToAssign courses from the shuffled array
        $assignedCourses = array_slice($courses, 0, $numCoursesToAssign);
          //echo "<script> console.log ('$assignedCourses') </script>";

         // Insert the assigned courses into a separate table, e.g., lecturer_courses
        foreach ($assignedCourses as $course) {
            $insertQuery = $dbh->prepare("INSERT INTO tblsuballocation (Teacherempid,subid,session_start,session_end,semesterid) VALUES (?, ?,?,?,?)");
            $insertQuery->execute([$lecturer['ID'], $course['ID'],$start,$end,$cid]);
        }
    }
if($insertQuery){
    echo "<script> alert('Allocation done') </script>";
}
//$i++;
//}
